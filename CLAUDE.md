...
> **📍 Doc sync:** CLAUDE.md last synced to commit `8d67249` — 2026-07-22 13:32 (Wed) [`construction_phase`].
...
- **🔭 FUTURE IMPROVEMENT (deferred — card thumbnails for grid speed).** _Symptom:_ the catalogue/home grids load the **full-size** product photos (the Zid images we re-hosted are ~1–2 MB each), so cards take 2–3s to actually paint on a slow connection. _Interim mitigation already shipped_ (`resources/js/components/store/product-image.tsx`): a branded placeholder (white + faint Retab logo) sits **permanently behind** the photo and the photo fades in over it, so the wait shows the logo, never a blank white square — but the bytes are still full-size. _Proper fix (do LATER, when the client hands over the **final designer-produced product images**):_ generate a **card-sized variant (~500px, WebP)** at write time and serve THAT in the grids, keeping the full image for the product-detail page. Scope: add a resize/variant step to **`App\Support\Media`** (`storeImage()`/`storeImageFromFile()` → also emit a `…-thumb.webp` next to the original; needs `intervention/image` or GD/Imagick), store/derive the thumb path on `product_images`, and point the catalogue card at it (`ShopController::card` ships the URL → `product-card.tsx`); re-run the importers so existing rows get thumbs. Free tier R2 easily covers the extra objects. **Trigger: the designer's final images are the signal to build this** — don't bother resizing the temporary Zid photos.
