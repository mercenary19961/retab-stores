...
> **📍 Doc sync:** CLAUDE.md last synced to commit `6aece1f` — 2026-07-15 10:44 (Wed) [`construction_phase`].
...
- **Arabic web font (BUILT 2026-07-02):** **Tajawal** via fonts.bunny.net, applied by overriding the Tailwind v4 token under RTL — `html[dir='rtl'] { --font-sans: 'Tajawal', ... }` in `app.css` (the `font-sans` utility resolves `var(--font-sans)`, so the token override cascades everywhere; no per-component changes). **SSR locale (BUILT):** `ssr.jsx` calls `i18n.changeLanguage(page.props.locale)` before render.
