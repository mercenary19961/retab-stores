...
- **Merge workflow — DECIDED (2026-07-04, same habit as Sky Amman): merge via the PR button in BOTH repos; NEVER push `main` from local.** Per feature: push the branch (dual-push delivers it identically to both repos) → open a PR in **each** repo → check CI → merge **both**. ⚠️ _Gotcha behind the rule:_ each repo's Merge button creates its own merge commit, so the two `main`s are **permanently SHA-divergent** (first instance: own `bc5a64c` vs client `f9934fa`, both merging `3055f1e`; Sky Amman's mains diverge the same way and have run fine in production). Harmless in this flow — the _content_ re-converges at every double-merge — but a plain `git push` of `main` will always be **rejected (non-fast-forward) on the client URL**. That's deliberate protection, not breakage: it makes it impossible to accidentally push `main` and trigger an instant Railway build; production deploys happen **only** at the intentional client-repo merge click. _The one real risk:_ **forgetting to merge one of the two PRs** → genuine content drift. If the repos ever seem out of sync, compare `git ls-remote <url> main` on both — expect different merge-commit SHAs but the same merged branch tips.
