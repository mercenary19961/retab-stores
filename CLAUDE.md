...
> **📍 Doc sync:** CLAUDE.md last synced to commit `1a23356` — 2026-07-23 10:31 (Thu) [`deployment_phase`].
...
- **⚠️ 301 redirects from the old Zid URLs — DO AT LAUNCH (don't skip).** The current Zid store is already indexed by Google; each ranked result points at an old Zid slug (captured as `product_page_url` in the Zid export → the `old_zid_slug` column in the cleaned import sheet). When we cut over, those URLs change → without redirects Google's links 404 and the accumulated ranking (link equity) is lost. **Fix:** build an `old_zid_slug → new product URL` map once the new URLs are settled, and serve **301 (permanent)** redirects (a `redirects` table or a config map + middleware). 301 both stops the 404s and passes the ranking to the new page. Keep the `old_zid_slug` column through migration for exactly this — it's the only reason it's there.
