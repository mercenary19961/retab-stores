...
> **📍 Doc sync:** CLAUDE.md last synced to commit `9fd5176` — 2026-07-02 16:52 (Thu) [`construction_phase`].
...
- [x] **CI (2026-07-02)** — GitHub Actions (`.github/workflows/ci.yml`): parallel jobs — PHP 8.2 + PHPUnit (in-memory SQLite) and Node 22 + **`tsc --noEmit`** (the real type gate; Vite/esbuild strips types without checking) + client + SSR builds. Enabler: **`withoutVite()` in `tests/TestCase.php`** — page-render tests no longer need a `public/build` manifest (gitignored → absent in CI; also retires the local "build before testing a new page" gotcha). _Still manual: branch protection on `main`; Playwright E2E later._ ⚠️ **CI is RED on GitHub (found 2026-07-04, unfixed)** — all runs of the `CI` workflow on `construction_phase` fail. _Symptom:_ frontend job dies at `tsc --noEmit` with `TS2307: Cannot find module 'ziggy-js'` (`resources/js/app.tsx:8`). _Root cause:_ `tsconfig.json` maps `ziggy-js` → `./vendor/tightenco/ziggy` (Ziggy ships its JS inside the composer package), but the Node-only CI job never runs `composer install`, so `vendor/` is absent and the path alias resolves to nothing. Locally tsc passes because `vendor/` exists; the Vite client/SSR builds pass even in CI because the `ziggy-js` import is type-only and esbuild strips it. _Fix options (decide + apply):_ (a) add `ziggy-js` as an npm devDependency pinned to the composer version (2.6.3) so tsc falls back to `node_modules` when the vendor path is missing, or (b) add PHP + `composer install --no-dev --no-scripts` to the frontend job. Backend (PHPUnit) job is green.
- [ ] Production deploy (MySQL, env vars, data-seeding migrations), Cloudflare DNS + Turnstile keys, mail domain verification

> **Last updated:** 2026-07-04 — **Status audit + doc corrections (no code changes).** Verified where the project stands: tree clean, `construction_phase` pushed everywhere, **141 tests / 501 assertions green locally** + local `tsc --noEmit` clean. Three doc corrections: (1) **client repo is populated + write access granted** — dual-push fully working, both branches in sync with origin (the "pending collaborator invite / repo empty" notes were stale); (2) **GitHub CI is RED** — `tsc` can't resolve `ziggy-js` in the Node-only job because the tsconfig alias points into `vendor/` (gotcha + fix options logged under Infrastructure → CI; fix not yet applied); (3) refreshed the doc-sync stamp (the two 2026-07-02 doc commits hadn't). Next up: fix CI, then merge `construction_phase` → `main` (PR #3 already merged up through the SSR/SEO commit; main is 4 commits behind — marketing, CI + Tajawal, 2 doc commits; pushing `main` will eventually drive the Railway prod deploy), then the Production-deploy checklist.
