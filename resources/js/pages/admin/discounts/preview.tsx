        ...
        <AdminLayout title={t('admin.discounts.preview.title')}>
                        ...
                        <tbody>
                            {diff.matched.map((m) => (
                                <tr key={m.product_id} className="border-b border-neutral-100 last:border-0 dark:border-neutral-800">
                                    ...
                                    <td className="px-4 py-2 whitespace-nowrap text-neutral-400 line-through">{money(m.price)}</td>
                                    <td className="px-4 py-2 whitespace-nowrap text-green-600 dark:text-green-400">-{m.percent}%</td>
                                    <td className="px-4 py-2 font-medium whitespace-nowrap text-neutral-800 dark:text-neutral-100">{money(m.new)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            {diff.unmatched.length > 0 && (
                <p className="mt-3 text-xs text-neutral-500">
                    {t('admin.discounts.preview.unmatchedSkus')}: {diff.unmatched.map((u) => u.sku).join(', ')}
                </p>
            )}
            ...
            <form onSubmit={apply} className="mt-6 rounded-lg border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900">
                ...
                <div className="mt-4 flex justify-end gap-2">
                    <Button type="button" variant="secondary" icon={X} href="/admin/discounts">
                        {t('admin.common.cancel')}
                    </Button>
                    <Button type="submit" variant="primary" icon={BadgePercent} disabled={form.processing || diff.matched.length === 0}>
                        {t('admin.discounts.preview.applyN', { n: diff.matched.length })}
                    </Button>
                </div>
            </form>
        </AdminLayout>
