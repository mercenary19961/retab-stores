...
/**
 * The return detail body (status, reason, items, photos, resolve actions, order
 * summary) shared by the full return page and the in-list modal. `onAction`
 * performs a verb (approve/reject/refund/exchange) with the built payload.
 */
export default function ReturnDetailView({
    ...
    onAction: (action: string, payload: Record<string, string | boolean>) => void;
    busy: boolean;
}) {
    const { t } = useAdminT();
